...
from study_server.db_definitions import ParticipantControl, FormDataControl
...
class StudyResource:
    ...
    def on_get(self, req: Request, resp: Response):  # noqa
        try:
            ...
            participantcontrol = db.query(
                ParticipantControl
            ).filter(
                ParticipantControl.id == session.id
            ).one_or_none()

            if participantcontrol is None:
                resp.content_type = falcon.MEDIA_TEXT
                resp.status = falcon.HTTP_BAD_REQUEST
                resp.text = "consent was not given to start the study"
                return
            ...
            if participantcontrol.extra is not None:
            ...
            try:
                with open(self.study_path, "rb") as file:
                    # read the contents of the file and replace the placeholders
                    contents = file.read()
                    contents = contents.replace(b"#PROLIFIC_ID#", participantcontrol.prolific_id.encode())
                    contents = contents.replace(b"#SESSION_ID#", session.id.encode())
                    # provide the HTML response
                    resp.content_type = falcon.MEDIA_HTML
                    resp.status = falcon.HTTP_OK
...
class SurveyResource:
    ...
    def on_post(self, req: Request, resp: Response):  # noqa
        try:
            ...
            db.query(
                ...
                ParticipantControl.extra.is_(None)
            ).update(
                {ParticipantControl.extra: req.media}
            )
    ...
    def on_get(self, req: Request, resp: Response):  # noqa
        try:
            ...
            if participantcontrol.survey is not None:
    ...
    def on_post(self, req: Request, resp: Response):  # noqa
        try:
            ...
            participantcontrol = db.query(
                ...
                ParticipantControl.survey.is_(None)
            ).one_or_none()
            ...
            participantcontrol.survey = req.media
            participantcontrol.end = pendulum.now()
    ...
    def on_get(self, req: Request, resp: Response):  # noqa
        try:
            ...
            if request.check is not None:
                ...
                if request.check != participantcontrol.prolific_id:
                    resp.content_type = falcon.MEDIA_TEXT
                    resp.status = falcon.HTTP_BAD_REQUEST
            ...
            try:
                with open(self.complete_path, "rb") as file:
                    # read the contents of the file and replace the placeholders
                    contents = file.read()
                    contents = contents.replace(b"#VERIFY_ID#", participantcontrol.prolific_id.encode())
                    # provide the HTML response
                    resp.content_type = falcon.MEDIA_HTML
                    resp.status = falcon.HTTP_OK
                    resp.text = contents
...
class RecordResource:
     def on_post(self, req, resp):
         try:
            session = req.env["beaker.session"]
            db = req.context.session
            form_data_dict_control = req.media
            print('form_data_dict_control',form_data_dict_control)
            formdatacontrol = FormDataControl(
                id=session.id,
                prolific_id=form_data_dict_control['prolificId'],
                round_number=form_data_dict_control['round'],
                first_form_value=form_data_dict_control['firstFormValue'],
                second_form_value=form_data_dict_control['secondFormValue']
            )
            # Implement your logic to save form_data into your database
            db.add(formdatacontrol)
            db.commit()
